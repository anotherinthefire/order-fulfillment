...
<head>
    ...
    <title>AXGG | Pickup</title>
    <link rel="shortcut icon" href="https://i.ibb.co/dfD3s4M/278104398-126694786613134-4231769107383237629-n-removebg-preview.png" />
    <link rel="stylesheet" href="./css/pickup-view.css">
</head>
...
<body>
    ...
    <section class="home-section">
        ...
        ?>
                ...
                <form action="actions/to-shipped.php" method="POST" style="display: inline;">
                    <input type="hidden" name="orderId" value="<?php echo $orderId; ?>">
                    <button style="margin-left: 65%; margin-bottom: 50px;" type="submit" name="ship" class="status">Shipped</button>
                </form>

                <button onclick="openModal()">Proof</button>

                <a href="pickup.php" style="color:black;">
                    <button>Back</button>
                </a>
                ...
                function insertPaymentImage($orderId, $imageFile)
                {
                    ...
                    if (move_uploaded_file($imageFile, $destination)) {
                        ...
                        if (mysqli_num_rows($existingResult) > 0) {
                            $updateSql = "UPDATE proof SET pickup_img = '$filename', pickup_date = '$timestamp' WHERE ord_id = $orderId";
                            ...
                            $insertSql = "INSERT INTO proof (ord_id, pickup_img, pickup_date) VALUES ($orderId, '$filename', '$timestamp')";
                ...
                if (isset($_FILES['file']) && isset($_POST['orderId'])) {
                    ...
                    $fileExtension = strtolower(pathinfo($_FILES['file']['name'], PATHINFO_EXTENSION));
                    $samepage = "http://localhost/overallsia/order-fulfillment/orders/pickup-view.php?id=" . $orderId;
                    if (in_array($fileExtension, $allowedExtensions)) {
                        if (insertPaymentImage($orderId, $imageFile)) {
                            echo '<script>alert("Pickup proof uploaded successfully."); window.location.href = "' . $samepage . '";</script>';
                        } else {
                            echo '<script>alert("Failed to upload pickup proof.");</script>';
                        }
                    } else {
    ...
    <script src="./js/pickup-view.js"></script>
</body>
