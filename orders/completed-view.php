    ...
    <section class="home-section">
        ...
        ?>
                ...
                <aside style="margin-left:5%; margin-right: 5%; padding-bottom:5%; text-align: center; ">
                    ...
                    // Query to retrieve remarks for the specified order ID
                    $sql = "SELECT remarks FROM proof WHERE ord_id = $orderId";
                    ...
                    if ($result && mysqli_num_rows($result) > 0) {
                        ...
                        // Retrieve the remarks from the row
                        $remarks = $row['remarks'];

                        // Display the remarks
                        echo '<h1 style="margin-top:50px;"><span>Remarks</span></h1>';
                        echo '<div style="margin-bottom: 50px;">';
                        echo '<p>' . $remarks . '</p>';
                        echo '</div>';

                        // Display the button to edit remarks
                        echo '<div id="remarksForm" style="display: none;">';
                        echo '<form action="actions/update-remarks.php" method="POST">';
                        echo '<textarea name="remarks">' . $remarks . '</textarea><br>';
                        echo '<input type="hidden" name="orderId" value="' . $orderId . '">';
                        echo '<button type="submit">Save Remarks</button>';
                        echo '</form>';
                        echo '</div>';
                    } else {
                        // No remarks found
                        echo '<h1><span>Remarks</span></h1>';
                        echo '<div style="margin-bottom: 50px;">';
                ...
                <form action="actions/to-refund.php" method="POST" style="display: inline;">
                    <input type="hidden" name="orderId" value="<?php echo $orderId; ?>">
                    <button style="margin-left: 60%; margin-bottom: 50px;" type="submit" name="refund" class="status">Refund</button>
                </form>

                <button onclick="openModal()">Proof</button>
                <button onclick="showRemarksForm()">Remarks</button>
                <a href="completed.php" style="color:black;">
                    <button>Back</button>
                </a>
                ...
                function insertPaymentImage($orderId, $imageFile)
                {
                    ...
                    if (move_uploaded_file($imageFile, $destination)) {
                        ...
                        if (mysqli_num_rows($existingResult) > 0) {
                            ...
                            $updateSql = "UPDATE proof SET remarks_img = '$filename', remarks_date = '$timestamp' WHERE ord_id = $orderId";
                            ...
                            $insertSql = "INSERT INTO proof (ord_id, remarks_img, remarks_date) VALUES ($orderId, '$filename', '$timestamp')";
                ...
                if (isset($_FILES['file']) && isset($_POST['orderId'])) {
                    ...
                    $fileExtension = strtolower(pathinfo($_FILES['file']['name'], PATHINFO_EXTENSION));
                    $samepage = "http://localhost/overallsia/order-fulfillment/orders/completed-view.php?id=" . $orderId;
                    if (in_array($fileExtension, $allowedExtensions)) {
                        if (insertPaymentImage($orderId, $imageFile)) {
                            echo '<script>alert("Remarks uploaded successfully."); window.location.href = "' . $samepage . '";</script>';
                        } else {
                            echo '<script>alert("Failed to upload remarks image.");</script>';
                        }
                    } else {
