...
<body>
  ...
  <section class="home-section">
    ...
    if (isset($_GET['search']) && !empty($_GET['search'])) {
      $search = $_GET['search'];
      $sql = "SELECT * FROM orders WHERE status = 8 AND (full_name LIKE '%$search%' OR contact LIKE '%$search%') ORDER BY ord_date DESC LIMIT $offset, $perPage";
    } else {
      $sql = "SELECT * FROM orders WHERE status = 8 ORDER BY ord_date DESC LIMIT $offset, $perPage";
    }
  ...
  <span class="icon"> 
    ...
    <tbody>';
      ...
      $totalOrders = mysqli_num_rows(mysqli_query($conn, "SELECT * FROM orders WHERE status = 8"));
