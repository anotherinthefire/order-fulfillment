...
<body>
  ...
  <section class="home-section">
    ...
    if (isset($_GET['search']) && !empty($_GET['search'])) {
      $search = $_GET['search'];
      $sql = "SELECT * FROM orders WHERE status = 1 AND (full_name LIKE '%$search%' OR contact LIKE '%$search%') ORDER BY ord_date DESC LIMIT $offset, $perPage";
    } else {
      $sql = "SELECT * FROM orders WHERE status = 1 ORDER BY ord_date DESC LIMIT $offset, $perPage";
    }
    ...
    <tbody>';
      ...
      <td>
        ...
        <div class="more-options">
          ...
          <form action="./actions/to-confirm.php" method="post">
            <input type="hidden" name="orderId" value="' . $orderId . '">
            <button type="submit" name="confirm" class="status">Confirm</button>
          </form>
          <a href="followup-view.php?id=' . $orderId . '"><button class="view">View</button></a>
        </div>
      </td>
    </tr>';
      }
      ...
      $totalOrders = mysqli_num_rows(mysqli_query($conn, "SELECT * FROM orders WHERE status = 1"));
